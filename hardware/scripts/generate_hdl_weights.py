...
def generate_layer_config_h(out_dir: str, act_lut_depth: int, model_path: str,
    ...
    The HLS top function reads this constexpr LAYER_CFG[] array to walk all
    17 layers in a single ap_start invocation.  Both the SVH and the H file
    are emitted from the same Python source so they cannot drift.
    """
    PP_CFG = {}
    for li in range(NUM_LAYERS):
        if li <= 10:
    ...
    # Derive HLS-friendly fields from the layer type
    def derive(hdl):
        is_1x1     = hdl.layer_type in (CONV1, CONV1_LIN)
        kh = kw    = 1 if is_1x1 else 3
        pad_h = pad_w = 0 if is_1x1 else 1
        use_silu   = 0 if hdl.layer_type == CONV1_LIN else 1
        use_pool   = 1 if hdl.layer_type == CONV3_POOL else 0
        return kh, kw, pad_h, pad_w, use_silu, use_pool
    ...
    type_enum = {
        CONV3:      "LT_CONV3",
        CONV3_POOL: "LT_CONV3_POOL",
        CONV1:      "LT_CONV1",
        CONV1_LIN:  "LT_CONV1_LIN",
    }
    ...
    for i, hdl in enumerate(HDL_LAYERS):
        ...
        zp_out = extracted[i].zp_out if extracted else 0
        pp_sel, pp_rd, pp_wr = PP_CFG[i]
        kh, kw, pad_h, pad_w, use_silu, use_pool = derive(hdl)
        sep = "," if i < NUM_LAYERS - 1 else ""
        lines.append(
            f"    /* {i:2d} */ {{ "
            f"{type_enum[hdl.layer_type]:<14s}, "
            f"{hdl.h_in:4d}, {hdl.w_in:4d}, "
            f"{hdl.cin:4d}, {hdl.cout:4d}, "
            f"{hdl.cin_groups:2d}, "
            f"0x{hdl.wt_base_addr:04x}, 0x{hdl.qp_base_addr:03x}, "
            f"{zp_in:5d}, {zp_out:5d}, "
            f"{pp_sel}, {pp_rd:3d}, {pp_wr:3d}, "
            f"{use_silu}, {use_pool}, {kh}, {kw}, {pad_h}, {pad_w}"
            f" }}{sep}"
        )
    lines.append("};")
    lines.append("")

    h_path = os.path.join(out_dir, "layer_config.h")
...
def main():
    ...
    parser.add_argument("--hls-out", default="hardware/hls",
                        help="Output directory for HLS layer_config.h "
                             "(default: hardware/hls/)")
    ...
    os.makedirs(args.hls_out, exist_ok=True)
    c_par = args.c_par

...
// ==============================================================
`timescale 1ns/1ps
(* DowngradeIPIdentifiedWarnings="yes" *) module tinyissimo_layer_top_control_s_axi
#(parameter
    ...
    output wire [31:0]                   in_h,
    output wire [31:0]                   in_w,
    output wire [31:0]                   in_c,
    output wire [31:0]                   out_c,
    output wire [31:0]                   kh,
    output wire [31:0]                   kw,
    output wire [31:0]                   pad_h,
    output wire [31:0]                   pad_w,
    output wire [0:0]                    use_maxpool,
    output wire [0:0]                    use_silu,
    output wire [31:0]                   layer_idx,
    output wire [7:0]                    zp_in,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_in_h = 'b0;
    reg  [31:0]                   int_in_w = 'b0;
    reg  [31:0]                   int_in_c = 'b0;
    reg  [31:0]                   int_out_c = 'b0;
    reg  [31:0]                   int_kh = 'b0;
    reg  [31:0]                   int_kw = 'b0;
    reg  [31:0]                   int_pad_h = 'b0;
    reg  [31:0]                   int_pad_w = 'b0;
    reg  [0:0]                    int_use_maxpool = 'b0;
    reg  [0:0]                    int_use_silu = 'b0;
    reg  [31:0]                   int_layer_idx = 'b0;
    reg  [7:0]                    int_zp_in = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_IN_H_DATA_0: begin
                    rdata <= int_in_h[31:0];
                end
                ADDR_IN_W_DATA_0: begin
                    rdata <= int_in_w[31:0];
                end
                ADDR_IN_C_DATA_0: begin
                    rdata <= int_in_c[31:0];
                end
                ADDR_OUT_C_DATA_0: begin
                    rdata <= int_out_c[31:0];
                end
                ...
                ADDR_USE_MAXPOOL_DATA_0: begin
                    rdata <= int_use_maxpool[0:0];
                end
                ADDR_USE_SILU_DATA_0: begin
                    rdata <= int_use_silu[0:0];
                end
                ADDR_LAYER_IDX_DATA_0: begin
                    rdata <= int_layer_idx[31:0];
                end
                ADDR_ZP_IN_DATA_0: begin
                    rdata <= int_zp_in[7:0];
                end
                ADDR_ZP_OUT_DATA_0: begin
                    rdata <= int_zp_out[7:0];
                end
                ADDR_WT_BASE_DATA_0: begin
                    rdata <= int_wt_base[31:0];
                end
...
assign in_h              = int_in_h;
assign in_w              = int_in_w;
assign in_c              = int_in_c;
assign out_c             = int_out_c;
assign kh                = int_kh;
assign kw                = int_kw;
assign pad_h             = int_pad_h;
assign pad_w             = int_pad_w;
assign use_maxpool       = int_use_maxpool;
assign use_silu          = int_use_silu;
assign layer_idx         = int_layer_idx;
assign zp_in             = int_zp_in;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_IN_H_DATA_0)
            int_in_h[31:0] <= (WDATA[31:0] & wmask) | (int_in_h[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_IN_W_DATA_0)
            int_in_w[31:0] <= (WDATA[31:0] & wmask) | (int_in_w[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_IN_C_DATA_0)
            int_in_c[31:0] <= (WDATA[31:0] & wmask) | (int_in_c[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_OUT_C_DATA_0)
            int_out_c[31:0] <= (WDATA[31:0] & wmask) | (int_out_c[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_use_maxpool[0:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_USE_MAXPOOL_DATA_0)
            int_use_maxpool[0:0] <= (WDATA[31:0] & wmask) | (int_use_maxpool[0:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_USE_SILU_DATA_0)
            int_use_silu[0:0] <= (WDATA[31:0] & wmask) | (int_use_silu[0:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_layer_idx[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_LAYER_IDX_DATA_0)
            int_layer_idx[31:0] <= (WDATA[31:0] & wmask) | (int_layer_idx[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_ZP_IN_DATA_0)
            int_zp_in[7:0] <= (WDATA[31:0] & wmask) | (int_zp_in[7:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_ZP_OUT_DATA_0)
            int_zp_out[7:0] <= (WDATA[31:0] & wmask) | (int_zp_out[7:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_WT_BASE_DATA_0)
            int_wt_base[31:0] <= (WDATA[31:0] & wmask) | (int_wt_base[31:0] & ~wmask);
    end
end

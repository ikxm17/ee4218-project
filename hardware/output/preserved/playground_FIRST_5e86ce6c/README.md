# playground_FIRST_5e86ce6c — HDL accelerator, first on-silicon working build

> **Status: sim-bit-exact, but NOT TFLite-faithful.** Weights/goldens in this
> build predate the round-half-up requantize fix. RTL matches its own sim
> golden bit-exactly, but both the RTL and the sim golden drift from TFLite
> by up to ±1 LSB at rounding boundaries. For TFLite-bit-exact behaviour use
> `playground_SECOND_3fcbe84a` or newer.
>
> **Scope: HDL accelerator only.** This is a PL-side-only build — no camera
> pipeline, no VDMA. Preprocessed images must be preloaded via AXI-Lite MMIO.
...
The FIRST build's timing report shows:
...
| File | Size | Purpose |
|------|------|---------|
| `playground_FIRST_5e86ce6c.bit` | 7.8 MB | Programming bitstream for PL |
| `playground_FIRST_5e86ce6c.xsa` | 2.3 MB | Hardware platform export (for PYNQ `Overlay()`) |
| `playground_FIRST_5e86ce6c.hwh` | 176 KB | Hardware handoff XML (PYNQ ip_dict) |
| `playground_FIRST_5e86ce6c.dtbo` | 196 B | Compiled device tree overlay |
| `playground_FIRST_5e86ce6c.dts` | 375 B | Device tree source |
| `playground_FIRST_5e86ce6c_routed.dcp` | 16.9 MB | Routed checkpoint (for Tcl forensics) |
| `playground_FIRST_5e86ce6c_timing_summary_routed.rpt` | 540 KB | Full timing summary (shows the 0.019 ns hold margin) |
| `playground_FIRST_5e86ce6c_utilization_placed.rpt` | 13 KB | Post-place resource utilization |

## Restoring this build

```bash
DIR=hardware/output/preserved/playground_FIRST_5e86ce6c
cp $DIR/playground_FIRST_5e86ce6c.bit  hardware/output/playground.bit
cp $DIR/playground_FIRST_5e86ce6c.xsa  hardware/output/playground.xsa
cp $DIR/playground_FIRST_5e86ce6c.hwh  hardware/output/playground.hwh
cp $DIR/playground_FIRST_5e86ce6c.dtbo hardware/output/playground.dtbo
cp $DIR/playground_FIRST_5e86ce6c.dts  hardware/output/playground.dts
